...
const text = 'Discover the Worlds Greatest Peaks'; // Your desired text
...
$(document).ready(function(){
  $('.dropdown').hover(function(){
    if($(window).width() > 768){ // Check if window width is greater than 768px (desktop)
  ...
  $('.dropdown').click(function(){
    if($(window).width() <= 768){ // Check if window width is less than or equal to 768px (mobile)
      $('#myDropdown').slideToggle();
    }
  });
});
let count = localStorage.getItem('visitorCount') || 0; // Retrieve count from localStorage or set to 0 if null
